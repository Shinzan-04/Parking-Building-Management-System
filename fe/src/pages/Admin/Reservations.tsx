...
import {
  ...
  CheckCircle2, XCircle, ClipboardList, RefreshCcw, LayoutGrid, ChevronDown
} from 'lucide-react';
...
import { getBuildings, getBuildingById, updateBuildingApprovalMode } from '../../services/buildingsService';
...
function ApprovalModeDropdown({ value, onChange, disabled }: any) {
  const [isOpen, setIsOpen] = useState(false);
  const options = [
    { value: 0, label: 'Manual' },
    { value: 1, label: 'Auto-Approve All' },
    { value: 2, label: 'Auto-Reject All' },
  ];
  const selectedLabel = options.find(o => o.value === value)?.label || 'Manual';

  return (
    <div className={`relative min-w-[140px] ${disabled ? 'opacity-50 pointer-events-none' : ''}`} tabIndex={0} onBlur={() => setIsOpen(false)}>
      <div 
        className="flex items-center justify-between gap-2 cursor-pointer select-none admin-text font-bold text-sm"
        onClick={() => setIsOpen(!isOpen)}
      >
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-[180px] bg-[var(--admin-bg-surface)] border border-[var(--admin-border)] rounded-2xl shadow-xl overflow-hidden z-50 py-1 font-normal">
          {options.map((opt) => (
            <div 
              key={opt.value}
              className={`px-4 py-2.5 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-colors ${value === opt.value ? 'admin-text font-bold' : 'admin-text-muted'}`}
...
export default function AdminReservations() {
  ...
  const [selectedBuildingId, setSelectedBuildingId] = useState<string>(user?.assignedBuildingId || '');
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const data = await getAllActiveReservations(bId);
  ...
  const handleModeChange = async (mode: number) => {
    ...
    try {
      await updateBuildingApprovalMode(selectedBuildingId, mode);
      setApprovalMode(mode);
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <p className="text-sm admin-text-muted">Loading reservation list...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          {selectedBuildingId && (
            <div className="flex items-center gap-2 bg-[var(--admin-bg-card)] px-4 py-2.5 rounded-xl border border-[var(--admin-border)]">
              <span className="text-sm font-semibold admin-text-faint">Approval Mode:</span>
          ...
          <div className="border border-[#FF4C4C]/20 rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-5 bg-white dark:bg-[#0E0E10]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 dark:admin-text-muted mt-0.5">The reservation will be accepted</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="border border-blue-400/20 rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-5 bg-white dark:bg-[#0E0E10]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-gray-800 dark:admin-text">Reassign Customer's Slot</h3>
                <p className="text-xs text-gray-400 dark:admin-text-muted mt-0.5">Resolve an occupied-slot issue</p>
              </div>
            </div>
            ...
            <div>
              ...
              {loadingSlots ? (
                ...
                >
                  <option value="" disabled style={{ color: '#000', backgroundColor: '#fff' }}>-- Please select a vacant slot --</option>
