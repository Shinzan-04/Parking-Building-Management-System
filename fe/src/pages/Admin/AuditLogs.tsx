...
/** Map ActionType sang label & màu dễ đọc */
function resolveAction(action: string): { label: string; color: string; bg: string; Icon: React.ElementType } {
  ...
  if (a.includes('create') || a.includes('add'))
    return { label: 'Tạo mới',       color: '#22c55e', bg: 'rgba(34,197,94,0.12)',  Icon: Plus       };
  if (a.includes('update') || a.includes('edit') || a.includes('approve') || a.includes('reject') || a.includes('cancel'))
    return { label: 'Cập nhật',      color: '#f59e0b', bg: 'rgba(245,158,11,0.12)', Icon: Pencil     };
  if (a.includes('delete') || a.includes('remove'))
    return { label: 'Xoá',           color: '#ef4444', bg: 'rgba(239,68,68,0.12)',  Icon: Trash2     };
  if (a.includes('force') || a.includes('admin'))
    return { label: 'Admin can thiệp', color: '#a78bfa', bg: 'rgba(167,139,250,0.12)', Icon: ShieldCheck };
  return { label: action, color: '#94a3b8', bg: 'rgba(148,163,184,0.12)', Icon: Tag };
}

/** Trả về label tiếng Việt của EntityName */
function entityLabel(name: string): string {
  const map: Record<string, string> = {
    subscription: 'Đăng ký vé tháng',
    pricingpolicy: 'Bảng giá',
    monthlypasspolicy: 'Chính sách vé tháng',
...
/** Parse JSON an toàn, trả về object hoặc null */
function safeJson(val: string | null | undefined): Record<string, unknown> | null {
  if (!val) return null;
  try { return JSON.parse(val); } catch { return null; }
}

/** Chỉ hiển thị các cặp key-value quan trọng, loại bỏ ID nội bộ */
...
function KeyValueList({ obj }: { obj: Record<string, unknown> }) {
  ...
  if (entries.length === 0) return <span className="text-xs opacity-50">Không có thay đổi đáng kể</span>;
  return (
      ...
      {expanded && hasDiff && (
        ...
        >
          <div className="p-3 space-y-2">
            ...
            {oldObj ? <KeyValueList obj={oldObj} /> : <span className="text-xs opacity-40">Không có dữ liệu cũ</span>}
          </div>
          <div className="p-3 space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: action.color }}>
              Sau thay đổi
            </p>
            {newObj ? <KeyValueList obj={newObj} /> : <span className="text-xs opacity-40">Không có dữ liệu mới</span>}
          </div>
        </div>
      )}
    </div>
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--admin-text-faint)' }}>
            Theo dõi mọi thay đổi quan trọng trong hệ thống — {logs.length} thao tác
          </p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Tìm người dùng, thao tác, đối tượng, lý do..."
            value={search}
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm">Không tìm thấy nhật ký nào.</p>
        </div>
      ) : (
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-center pb-2" style={{ color: 'var(--admin-text-faint)' }}>
          Hiển thị {filtered.length} / {logs.length} thao tác
        </p>
      )}
    </div>
  );
