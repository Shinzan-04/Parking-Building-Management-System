...
export default function Vehicles() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setApiError(err instanceof Error ? err.message : 'Unable to load vehicle data.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const stats = [
    { label: 'Total Vehicles Today', value: totalToday,             unit: 'vehicles',  icon: TrendingUp,  color: '#FF4C4C', bg: 'from-[#FF4C4C]/20 to-[#FF4C4C]/5' },
    { label: 'Currently Parked',     value: parkedCount,            unit: 'vehicles',  icon: Car,         color: '#FF4C4C', bg: 'from-[#FF4C4C]/20 to-[#FF4C4C]/5' },
    { label: 'Exited Today',         value: exitedCount,            unit: 'vehicles',  icon: CheckCircle2,color: '#A78BFA', bg: 'from-violet-400/20 to-violet-400/5' },
    { label: 'Revenue Today',        value: `${vnd(totalRevToday)}đ`, unit: '',  icon: TrendingUp,  color: '#FF4C4C', bg: 'from-[#FF4C4C]/20 to-[#FF4C4C]/5' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-white/40 mt-0.5">
            Track vehicle entry / exit · Today {new Date().toLocaleDateString('vi-VN')}
          </p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            type="text"
            placeholder="Search license plate, driver, building..."
            value={search}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                <th className="text-left text-xs font-medium text-white/40 px-6 py-3.5">License Plate</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Driver</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Vehicle Type</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Building / Zone</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Entry Time</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Exit Time</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Duration</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Fee</th>
                <th className="text-left text-xs font-medium text-white/40 px-4 py-3.5">Status</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
        ...
        {filtered.length > 0 && (
          <div className="px-6 py-3 border-t border-white/5 flex items-center justify-between">
            <p className="text-xs text-white/30">
              Showing {filtered.length} / {totalToday} vehicles
            </p>
            <p className="text-xs text-white/30">
              {parkedCount} parked · {exitedCount} exited
            </p>
          </div>
        )}
      </div>
